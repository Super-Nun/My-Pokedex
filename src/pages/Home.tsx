            ...
            {search && filteredPokemon.length > 0 ? (
              <div className="grid grid-rows justify-center gap-2 ">
                {filteredPokemon.map((p) => (
                  <div
                    key={p.id}
                    className="container bg-blue-100 inset-shadow- inset-shadow-gray-600 w-4xl pt-5 pb-5 rounded-lg text-center flex flex-row cursor-pointer hover:scale-110 transition-transform"
                  ...
                  >
                    ...
                    {/* Display Card Details Based on Supertype */}
                    {p.supertype === 'Pokémon' ? (
                      ...
                      <div className="ps-10 pe-10 text-gray-600 space-y-5 flex flex-col justify-center text-justify w-2xl">
                        ...
                        <p className="font-mono">
                          {Array.isArray(p.text) ? p.text.join(' ') : p.ability?.text || 'NaN'}
                        </p>
                      </div>
                    ) : p.supertype === 'Energy' ? (
                      <div className="ps-10 pe-10 text-gray-600 space-y-5 flex flex-col justify-center text-justify w-2xl">
                        ...
                        <p className="font-mono">
                          {Array.isArray(p.rules) ? p.rules.join(' ') : p.text || 'NaN'}
                        </p>
                      </div>
                    ) : null}
                  </div>
